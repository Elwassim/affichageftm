-- ================================================
-- CORRIGER TOUS LES PROBLEMES RLS - CGT FTM
-- ================================================
-- Script complet pour corriger toutes les permissions RLS
...
-- 2. CRÉER TOUTES LES FONCTIONS RPC BYPASS

-- Utilisateurs
CREATE OR REPLACE FUNCTION get_all_users()
RETURNS TABLE (
  id UUID, username VARCHAR, email VARCHAR, role VARCHAR,
  is_admin BOOLEAN, is_active BOOLEAN,
  created_at TIMESTAMP WITH TIME ZONE, updated_at TIMESTAMP WITH TIME ZONE
)
SECURITY DEFINER LANGUAGE SQL AS $$
  SELECT id, username, email, role, is_admin, is_active, created_at, updated_at
...
RETURNS TABLE (
  id UUID, name VARCHAR, photo TEXT, text TEXT,
...
-- Test si tables vides et insérer exemples
DO $$
BEGIN
  -- Utilisateur admin par défaut
  IF NOT EXISTS (SELECT 1 FROM users WHERE role = 'admin') THEN
    INSERT INTO users (username, password_hash, email, role, is_admin, is_active) VALUES
    ('admin', '$2b$10$defaulthash', 'admin@cgt-ftm.fr', 'admin', true, true);
  END IF;

  -- Catégories permanences par défaut
  IF NOT EXISTS (SELECT 1 FROM permanence_categories) THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM permanences) THEN
    INSERT INTO permanences (name, type, category, month, year, days, description) VALUES
    ('TEST, Admin', 'technique', 'P', 'janvier', 2025, '{"1":"P","15":"P"}', 'Permanence de test');
  END IF;

  -- Hommage d'exemple
  IF NOT EXISTS (SELECT 1 FROM tributes) THEN
    INSERT INTO tributes (name, photo, text, date_added) VALUES
    ('Hommage Test', '', 'Exemple d''hommage pour vérifier la synchronisation.', NOW());
  END IF;
...
UNION ALL
SELECT 'PERMANENCES', COUNT(*) FROM permanences
UNION ALL
SELECT 'CATEGORIES', COUNT(*) FROM permanence_categories
UNION ALL
SELECT 'HOMMAGES', COUNT(*) FROM tributes
UNION ALL
SELECT 'RÉUNIONS', COUNT(*) FROM meetings;

-- 5. TEST DES FONCTIONS RPC
SELECT 'Test RPC Users: ' || COUNT(*) as test FROM get_all_users();
SELECT 'Test RPC Permanences: ' || COUNT(*) as test FROM get_all_permanences();
SELECT 'Test RPC Catégories: ' || COUNT(*) as test FROM get_all_permanence_categories();
SELECT 'Test RPC Hommages: ' || COUNT(*) as test FROM get_all_tributes();
SELECT 'Test RPC Réunions: ' || COUNT(*) as test FROM get_all_meetings();

SELECT '✅ SYNCHRONISATION COMPLÈTE TERMINÉE!' as final_status;
